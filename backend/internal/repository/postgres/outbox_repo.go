...
func (r *OutboxRepository) Insert(ctx context.Context, tx *sqlx.Tx, eventID, eventType string, payload interface{}) error {
	...
	query := `INSERT INTO outbox (event_id, event_type, payload, status) VALUES ($1, $2, $3, 'pending')`
	_, err = tx.ExecContext(ctx, query, eventID, eventType, data)
	return err
}

func (r *OutboxRepository) GetPending(ctx context.Context, limit int) ([]OutboxEvent, error) {
	query := `SELECT id, event_id, event_type, payload, created_at, processed_at, status, retry_count, last_error, next_retry_at
	          FROM outbox
